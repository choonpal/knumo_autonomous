...
void UbloxFirmware8::getRosParams() {
  ...
  if (!node_->get_parameter(
        "rtcm_input.status_timeout_sec", rtcm_status_timeout_sec_) ||
      !std::isfinite(rtcm_status_timeout_sec_) ||
      rtcm_status_timeout_sec_ <= 0.0) {
...
void UbloxFirmware8::initializeRosDiagnostics() {
  ...
  if (getRosBoolean(node_, "publish.rxm.rtcm")) {
    freq_rtcm_ = std::make_unique<UbloxTopicDiagnostic>(
        std::string("rxmrtcm"), kRtcmFreqMin, kRtcmFreqMax,
        kRtcmFreqTol, kRtcmFreqWindow, updater_);
    updater_->add("RTCM Input", this,
                  &UbloxFirmware8::rtcmInputDiagnostics);
    updater_->force_update();
  }
}

void UbloxFirmware8::callbackRxmRtcm(
    const ublox_msgs::msg::RxmRTCM &m) {
  rxm_rtcm_pub_->publish(m);
  {
    std::lock_guard<std::mutex> lock(rtcm_status_mutex_);
    last_rxm_rtcm_ = m;
    last_rxm_rtcm_reception_ = std::chrono::steady_clock::now();
    rtcm_received_ = true;
  }
  if (freq_rtcm_) {
    freq_rtcm_->diagnostic->tick();
  }
  updater_->force_update();
}

void UbloxFirmware8::rtcmInputDiagnostics(
    diagnostic_updater::DiagnosticStatusWrapper& stat) {
  ublox_msgs::msg::RxmRTCM last_rxm_rtcm;
  std::chrono::steady_clock::time_point last_reception;
  {
    std::lock_guard<std::mutex> lock(rtcm_status_mutex_);
    if (!rtcm_received_) {
      stat.level = diagnostic_msgs::msg::DiagnosticStatus::ERROR;
      stat.message = "No RTCM input observed";
      return;
    }
    last_rxm_rtcm = last_rxm_rtcm_;
    last_reception = last_rxm_rtcm_reception_;
  }

  const double age_sec =
    std::chrono::duration<double>(
      std::chrono::steady_clock::now() - last_reception).count();
  stat.add("RTCM reception age (s)", age_sec);
  stat.add("Reference station ID", last_rxm_rtcm.ref_station);
  stat.add("RTCM message type", last_rxm_rtcm.msg_type);

  if (age_sec > rtcm_status_timeout_sec_) {
    stat.level = diagnostic_msgs::msg::DiagnosticStatus::ERROR;
    stat.message = "RTCM input status stale";
    return;
  }

  if (last_rxm_rtcm.flags &
      ublox_msgs::msg::RxmRTCM::FLAGS_CRC_FAILED) {
    stat.level = diagnostic_msgs::msg::DiagnosticStatus::ERROR;
    stat.message = "RTCM CRC failed";
  } else {
    stat.level = diagnostic_msgs::msg::DiagnosticStatus::OK;
    stat.message = "RTCM input valid";
  }
}
...
void UbloxFirmware8::subscribe(std::shared_ptr<ublox_gps::Gps> gps) {
  ...
  if (getRosBoolean(node_, "publish.rxm.rtcm")) {
    gps->subscribe<ublox_msgs::msg::RxmRTCM>(
        std::bind(&UbloxFirmware8::callbackRxmRtcm, this,
