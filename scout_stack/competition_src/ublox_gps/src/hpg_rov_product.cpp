...
HpgRovProduct::HpgRovProduct(uint16_t nav_rate, std::shared_ptr<diagnostic_updater::Updater> updater, rclcpp::Node* node)
  : dgnss_mode_(ublox_msgs::msg::CfgDGNSS::DGNSS_MODE_RTK_FIXED), updater_(updater), node_(node)
{
  (void)nav_rate;
...
void HpgRovProduct::carrierPhaseDiagnostics(
  ...
  const bool fix_ok =
      last_rel_pos_.flags & ublox_msgs::msg::NavRELPOSNED::FLAGS_GNSS_FIX_OK;
  const bool differential =
      last_rel_pos_.flags & ublox_msgs::msg::NavRELPOSNED::FLAGS_DIFF_SOLN;
  const bool relative_position_valid =
      last_rel_pos_.flags & ublox_msgs::msg::NavRELPOSNED::FLAGS_REL_POS_VALID;
  if (!fix_ok || !differential || !relative_position_valid ||
      carr_soln == ublox_msgs::msg::NavRELPOSNED::FLAGS_CARR_SOLN_NONE) {
    stat.level = diagnostic_msgs::msg::DiagnosticStatus::ERROR;
    stat.message = "None";
  } else if (carr_soln ==
             ublox_msgs::msg::NavRELPOSNED::FLAGS_CARR_SOLN_FLOAT) {
    stat.level = diagnostic_msgs::msg::DiagnosticStatus::WARN;
    stat.message = "Float";
  } else if (carr_soln ==
             ublox_msgs::msg::NavRELPOSNED::FLAGS_CARR_SOLN_FIXED) {
    stat.level = diagnostic_msgs::msg::DiagnosticStatus::OK;
    stat.message = "Fixed";
  } else {
    stat.level = diagnostic_msgs::msg::DiagnosticStatus::ERROR;
    stat.message = "Unknown carrier-phase state";
  }
