...
class UbloxFirmware7Plus : public UbloxFirmware {
 public:
  ...
  void callbackNavPvt(const NavPVT& m) {
    ...
    // Set the Fix status
    const bool fix_ok = m.flags & m.FLAGS_GNSS_FIX_OK;
    const bool has_position_fix =
        m.fix_type == m.FIX_TYPE_2D ||
        m.fix_type == m.FIX_TYPE_3D ||
        m.fix_type == m.FIX_TYPE_GNSS_DEAD_RECKONING_COMBINED;
    if (fix_ok && has_position_fix) {
 ...
 protected:
  ...
  void fixDiagnostic(diagnostic_updater::DiagnosticStatusWrapper& stat) override {
    ...
    if (last_nav_pvt_.fix_type ==
      ...
      stat.message = "Time only; no position fix";
    } else {
      stat.level = diagnostic_msgs::msg::DiagnosticStatus::ERROR;
      stat.message = "Unknown fix type";
    }
    ...
    if (!(last_nav_pvt_.flags & ublox_msgs::msg::NavPVT::FLAGS_GNSS_FIX_OK)) {
      if (stat.level < diagnostic_msgs::msg::DiagnosticStatus::WARN) {
        stat.level = diagnostic_msgs::msg::DiagnosticStatus::WARN;
      }
